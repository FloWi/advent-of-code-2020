...
object HexObj {
  ...
  /** Convert x/y (web mercator) to hex nume value.
    ...
    */
  def toNume(x: Double, y: Double, size: Double, orig: Double = -20000000.0, orientation: Orientation = Orientation.TOP_FLAT): Long = {
    val px = (x - orig) / size
    val py = (y - orig) / size
  ...
  /** Convert hex nume to X/Y meter values.
    ...
    */
  def toXY(nume: Long, size: Double, orig: Double = -20000000.0, orientation: Orientation = Orientation.TOP_FLAT): (Double, Double) = {
    val q = nume >> 32
