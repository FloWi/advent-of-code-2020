...
object part2 {
  ...
  def solve(input: String): Long = {
    ...
    val maxValue = parsed.max
    val rest = (maxValue + 1).to(1000 * 1000)
    val initial = parsed ++ rest

    val finalState = play(initial, 10 * 1000 * 1000).last._2

    //After the crab is done, what order will the cups be in?
    //Starting after the cup labeled 1, collect the other cups' labels clockwise into a single string with no extra characters;
    //each number except 1 should appear exactly once.

    val firstTwoNumbersAfter1 = putThisCupFirst(1, finalState).take(2)
    val result = firstTwoNumbersAfter1.map(_.toLong).product
    result
  }
